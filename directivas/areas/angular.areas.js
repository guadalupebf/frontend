...
app._areas = {};
app.areaid = -1;
app.directive("ngAreas", ['$parse', function ($parse, $scope) {
	return {
		restrict : 'A',
		...
		link : function($scope, $element, $attrs) {
			...
			var reload = function(){
				...
				console.log('global Areas: '+JSON.stringify(app._areas));
			}
			...
			$scope.$on("ngAreas:reset", function(event){
				mainImageSelectAreas.reset()
			});
		   ...
		   $scope.imageArea = function(parent, areaid) {
			   if (!$scope.name_for_new_tag) return;
